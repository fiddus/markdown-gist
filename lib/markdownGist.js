    ...
    module.exports = {
        ...
        makeGistFiles: function (fileName, blocks) {
            ...
            _.forEach(blocks, function (block) {
                var newFile = {},
                    blockHead = block.match(/^```(?:\w)$/gm) ? block.match(/^```(?:\w)$/gm).shift() : '```',
        ...
        mountWordpressPost: function (contents, originalFile) {
            ...
            _.forEach(content.files, function (gistFile) {
                var fileType = gistFile.language.toLocaleLowerCase() === 'text' ? '' : gistFile.language.toLowerCase(),
                    replacer = '```'
                        + fileType
                        + '\n' + gistFile.content
                        + '```',
        ...
        parseGitHubMarkdownFile: function (filePath) {
            sequence
                ...
                .then(function (next, err) {
                    ...
                    var lastBarIndex = filePath.lastIndexOf('/');

                    fileContent = module.exports.readFile(filePath, encoding);

                    if (lastBarIndex > -1) {
                        filePath = filePath.substr(lastBarIndex + 1);
                    }
                ...
                .then(function (next, err) {
                    ...
                    if (_.isEmpty(gistFiles)) {
                        console.log('Error: There are no blocks make with GitHub Markdown systax.');
